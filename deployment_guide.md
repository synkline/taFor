...
## 7. PDF Storage & Server Deployment

The application supports PDF generation. For future server-side storage integration:
- A local folder `savedTAFs/` is used to store generated TAF reports.
- Ensure the application user (e.g., `www-data` or your username) has write permissions to this directory:
  ```bash
  mkdir -p savedTAFs
  sudo chown -R $USER:www-data savedTAFs
  sudo chmod -R 775 savedTAFs
  ```
